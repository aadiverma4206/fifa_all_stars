...
export const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-colors overflow-x-clip">
      <div className="max-w-[1600px] w-full mx-auto px-3 sm:px-8 lg:px-12">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <nav className="hidden xl:flex items-center space-x-1 sm:space-x-1.5">
            {currentUser ? (
            ...
            {currentUser ? (
              <>
                ...
                {userRole === 'PLAYER' && (
                  <Link 
                    to="/player/wallet" 
                    className="flex items-center space-x-1.5 px-3 py-1 rounded-md bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 text-xs font-bold hover:bg-amber-500/20 hover:scale-105 active:scale-95 transition-all shadow-sm group"
                    title="Click to Add Money to Wallet"
                  >
                    <Wallet className="w-3.5 h-3.5 text-amber-500 group-hover:scale-110 transition-transform" />
                    ...
                    <div className="absolute right-0 mt-2 w-64 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl py-2 z-50 text-xs font-semibold space-y-1">
                      ...
                      {userRole === 'PLAYER' && (
                        <Link
                          to="/player/wallet"
                          onClick={() => setProfileDropdownOpen(false)}
                          className="flex items-center justify-between px-4 py-2.5 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors font-bold"
                        >
          ...
          {/* Mobile & Tablet menu button & quick action icons */}
          <div className="flex xl:hidden items-center space-x-1.5 sm:space-x-2 flex-shrink-0">
            {/* Wallet Balance Badge for Players on Mobile/Tablet */}
            {userRole === 'PLAYER' && (
              <Link 
                to="/player/wallet" 
                className="flex items-center space-x-1 px-2.5 py-1 rounded-md bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 text-xs font-bold hover:bg-amber-500/20 transition-all"
                title="Wallet & Add Money"
              >
                <Wallet className="w-3.5 h-3.5 text-amber-500" />
                <span className="font-mono">₹{currentUser?.walletBalance?.toFixed(0) || '0'}</span>
              </Link>
            )}
                      ...
                      <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700/80 flex items-center justify-between shadow-xs">
                        ...
                        {userRole === 'PLAYER' && (
                          <Link 
                            to="/player/wallet" 
                            onClick={() => setMobileMenuOpen(false)} 
                            className="flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-500 text-xs font-mono font-bold flex-shrink-0 hover:bg-amber-500/20"
                            title="Click to Add Money to Wallet"
                          >
                            <Wallet className="w-3.5 h-3.5 text-amber-500" />
                            <span>₹{currentUser?.walletBalance?.toFixed(0) || '0'}</span>
                          </Link>
                        )}
                      </div>
